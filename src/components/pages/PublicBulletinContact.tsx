...
function NewsAttachmentPreview({ post }: { post: NewsPost }) {
  ...
  return (
    <div className="mt-4 space-y-3">
      {post.attachments.map((attachment) => (
        <div
          key={`${post.id}-${attachment.name}`}
          className="overflow-hidden rounded-2xl border border-[color:color-mix(in_oklab,var(--border)_72%,var(--yor-copper)_28%)] bg-black/10"
        >
          {hasRenderablePreview(attachment) ? (
            attachment.kind === 'image' ? (
              <img src={attachment.dataUrl} alt={attachment.name} className="max-h-72 w-full object-cover" />
            ) : (
              <video src={attachment.dataUrl} controls className="max-h-72 w-full bg-black object-cover" />
            )
          ) : null}
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 text-sm">
            ...
            <a
              ...
              className="rounded-full border border-[color:color-mix(in_oklab,var(--yor-copper)_38%,var(--border))] px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--yor-copper-soft)]"
            >
              Download
            </a>
          </div>
  ...
  return (
    <section className="story-section" id="bulletin">
      <div className="page-container">
        <span className="eyebrow">Bulletin</span>
        <h2 className="display-heading">Latest from Yor International</h2>
        <p className="hero-summary">Announcements, promos, and company news published straight from the Yor office.</p>
        <div className="mt-5">
          <Link
            to="/news-updates"
            className="inline-flex items-center rounded-full border border-[color:color-mix(in_oklab,var(--yor-copper)_36%,var(--border))] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--yor-copper-soft)]"
          >
            View all updates
          </Link>
        </div>
            ...
            >
              ...
              <div className="relative">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    {NEWS_CATEGORY_LABEL[featured.category] ?? 'Post'}
                  </span>
                  {featured.pinned ? (
                ...
                <div
                  className="news-rich-body mt-3 text-sm leading-relaxed text-[var(--muted-foreground)]"
                  dangerouslySetInnerHTML={{ __html: renderNewsBodyHtml(featured.body) }}
                />
                <NewsAttachmentPreview post={featured} />
              </div>
              {featured.createdByLabel ? (
                <p className="relative mt-6 text-[11px] uppercase tracking-[0.2em]" style={{ color: 'var(--yor-copper-soft)' }}>
                  By {featured.createdByLabel}
                </p>
              ) : null}
            </article>
          ) : null}
          ...
          <div className="grid content-start gap-4">
            {rest.slice(0, 4).map((post, index) => (
              <article
                ...
                style={{ animation: 'yor-rise 0.6s ease both', animationDelay: `${0.08 * (index + 1)}s` }}
              >
                <div className="flex items-center justify-between gap-3">
                  <span className="text-[10px] font-semibold uppercase tracking-[0.22em]" style={{ color: 'var(--yor-copper-soft)' }}>
                    {NEWS_CATEGORY_LABEL[post.category] ?? 'Post'}
                  </span>
                  <span className="text-[11px]" style={{ color: 'var(--muted-foreground)' }}>
                    {formatNewsDate(post.publishedAt)}
                  </span>
                </div>
                <h4 className="mt-2 text-base font-semibold leading-snug" style={{ color: 'var(--foreground)' }}>
                  {post.title}
                </h4>
                <p className="mt-1.5 line-clamp-3 text-sm leading-relaxed" style={{ color: 'var(--muted-foreground)' }}>
                  {stripNewsFormatting(post.body)}
                </p>
              </article>
            ))}
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            ...
            <p className="hero-summary">
              Questions about packages, the compensation plan, or partnering with Yor International? Send us a
              message and the office team will get back to you.
            </p>
            <div className="mt-8 grid gap-4">
              {[
                ...
                <div key={label} className="flex items-baseline gap-4">
                  <span
                    className="w-16 shrink-0 text-[10px] font-semibold uppercase tracking-[0.22em]"
                    style={{ color: 'var(--yor-copper-soft)' }}
                  >
                    {label}
                  </span>
            ...
            {status === 'sent' ? (
              <div className="flex flex-col items-center gap-4 py-10 text-center">
                ...
                <p className="max-w-sm text-sm" style={{ color: 'var(--muted-foreground)' }}>
                  Thank you for reaching out. The Yor office team has received your message and will reply to your
                  email soon.
                </p>
                <button
                  type="button"
              ...
              <form className="grid gap-4" onSubmit={handleSubmit}>
                {error ? (
                  <div
                    ...
                    style={{
                      border: '1px solid color-mix(in oklab, red 40%, var(--border))',
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="grid gap-1.5">
                    <span className="text-[11px] font-semibold uppercase tracking-[0.18em]" style={{ color: 'var(--yor-copper-soft)' }}>
                      Name
                    </span>
...
export function PublicBulletinContact() {
  return (
    <>
      <style>{`
        @keyframes yor-rise { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
        .news-rich-body p + p { margin-top: 0.95rem; }
        .news-rich-body ul { margin: 0.75rem 0 0; padding-left: 1.1rem; }
        .news-rich-body li + li { margin-top: 0.35rem; }
      `}</style>
